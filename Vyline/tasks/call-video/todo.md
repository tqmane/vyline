...
- [x] call-recording: 所有者・チャンク・保存・容量・中断・Range・削除・保持の隔離テストと実装
- [x] recording-storage: GUIの保存先選択、サーバー許可パス／マウント済み外部ストレージ／WebDAVと転送失敗時の保持。実NASではなく隔離WebDAVで確認
- [x] call-layout: Zoom参考のギャラリー／フォーカス／固定／ページ・タップ操作と音声の縦スクロール
- [x] call-recording: 通話音声ミックス→録音、映像合成→録画、停止後の種別切替・自動開始／終了
- [x] recording-library: 設定から一覧・再生・DL・削除・自動設定・30日／無期限の保持
- [x] call-docs: 最新コードを正本に詳細技術仕様と新しい保存APIを記録
- [x] 全体テスト・型・Lint・build・ブラウザー・隔離BFF保存の検証。632件成功/0失敗、既存bundle警告のみ。追加8件でWebDAV実30秒timeout・Range・Cookie認証も確認
...
- [x] 追加依頼: 映像分割位置のドラッグ・キーボード調整、大画面の独立通話ペイン、スマホで畳んでトークを操作するUI。320×480〜1440pxで終了操作可視、合成ポインターとキー、同じ媒体DOM・trackの維持を実ブラウザー確認
