...
- [ ] 着信バナーが再読み込みなしで出ることを実画面で確認
- [x] chat-refresh: 2箇所のボタン・失敗通知・状態保持テスト
- [ ] chat-refresh: 全体検証、狭い画面/デスクトップ実表示、PR・配布
- [x] video-media: ネイティブの初期フラグ・pmap・MCMMDの静的調査
- [ ] video-media: 制御されたWindows観測でEVS3のフレーム/分割形式を確定
- [ ] video-media: 型契約・パケット化/復元と不正入力テスト
- [ ] Checkpoint: 映像transportのfixture検証、既存音声テスト維持
- [ ] video-media: 認証済み動画フレーム経路・ライフサイクル
- [ ] video-media: ブラウザ符号化/復号、映像表示、カメラ操作
- [ ] video-upgrade: 同一通話上の開始/停止制御と切替UI
