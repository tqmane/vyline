...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    response.setHeader("Content-Type", ({ ".wasm": "application/wasm", ".js": "text/javascript", ".html": "text/html;charset=utf-8", ".ttf": "font/ttf" })[extname(path)] ?? "application/octet-stream");
    response.end(bytes);
...
try {
  ...
  assert.equal(await page.evaluate(() => window.frameError), undefined, "font loading must succeed");
  const state = {
    mode: "apple", dark: false, reducedMotion: false, query: "", tab: "all",
    tabs: [{ id: "all", label: "すべて" }, { id: "unread", label: "未読" }, { id: "groups", label: "グループ" }],
    rows: Array.from({ length: 1200 }, (_, index) => ({ id: `chat-${index}`, title: `${["高橋 美咲", "週末の予定", "Vyline 開発", "写真を共有", "山田 太郎"][index % 5]} ${index + 1}`,
      preview: ["明日は何時に集合する？", "写真を送信しました", "このUI、いい感じですね", "ありがとう！また後で連絡します"][index % 4], time: index % 2 ? "昨日" : "12:34", unread: index % 4 === 0 ? 3 : 0,
      avatar: ["高", "週", "V", "写", "山"][index % 5], color: "#7C98B8", selected: index === 1, pinned: index === 2, muted: false, locked: false })),
    profile: { name: "検証ユーザー", status: "" }, sortLabel: "新しい順", canRefresh: true, splitPick: false,
  };
  ...
  if (chatMode) Object.assign(state, {
    chat: { id: "chat-0", title: "高橋 美咲", status: "オンライン", avatar: "高", color: "#7C98B8", isGroup: false, locked: false, blocked: false },
  ...
  for (const mode of ["apple", "fluent", "miuix"]) {
    ...
    if (chatMode && process.argv.includes("--slots")) {
      ...
      await page.waitForFunction(() => window.slotEvents.some((event) => event.type === "content-slot" && event.id === "shared-content" && event.chatId === "chat-0" && event.epoch === 1));
      await frame.waitForFunction(() => window.__vylineMessageSlots?.get("shared-content")?.isConnected);
      ...
      assert.ok(Math.abs(slot.height - 245) < 1 && slot.width <= 360, "inline shared content must follow the measured native layout");
      await page.evaluate(() => window.sendPatch({ hostContentHeights: { "shared-content": 312 } }));
      await frame.waitForFunction(() => Math.abs(window.__vylineMessageSlots.get("shared-content").getBoundingClientRect().height - 312) < 1);
      await page.evaluate(() => window.sendPatch({ chat: null, messages: [] }));
      await page.waitForFunction(() => window.slotEvents.some((event) => event.type === "content-slot-removed" && event.id === "shared-content"));
      assert.equal(await frame.evaluate(() => window.__vylineMessageSlots.has("shared-content")), false);
    ...
    if (chatMode && process.argv.includes("--delta")) {
      assert.equal(await page.evaluate(() => window.messageDelta), true, "renderer must advertise delta support");
      const added = { ...state.messages.at(-1), id: `delta-${mode}`, text: "差分で受信したメッセージ", createdAt: state.messages.at(-1).createdAt + 1000 };
      await page.evaluate(({ updates, ids }) => window.sendPatch({ messageDelta: { updates, ids } }), { updates: [added], ids: [...state.messages.map((message) => message.id), added.id] });
      await expect(frame.getByRole("list", { name: "メッセージ履歴" })).toContainText("差分で受信したメッセージ");
      await page.evaluate((message) => window.sendPatch({ messageDelta: { updates: [message] } }), { ...added, text: "差分を更新しました" });
      await expect(frame.getByRole("list", { name: "メッセージ履歴" })).toContainText("差分を更新しました");
      await page.evaluate((ids) => window.sendPatch({ messageDelta: { updates: [], ids } }), state.messages.map((message) => message.id));
      await expect(frame.getByRole("list", { name: "メッセージ履歴" })).not.toContainText("差分を更新しました");
      console.log(`${mode}: delta append/update/remove passed`);
    }
    const accessible = await frame.locator("body").ariaSnapshot();
    assert.match(accessible, /高橋 美咲/, "Japanese conversation labels must reach the accessibility tree");
    assert.ok(await frame.getByRole("button").count() < 80, "1200 conversations must remain virtualized");
    if (chatMode) {
      await clickNative(frame.getByRole("textbox", { name: "メッセージを入力", exact: true }));
      await page.keyboard.insertText("確認メッセージ");
      await page.waitForFunction(() => window.actions.some((item) => item.action === "draft" && item.value === "確認メッセージ"));
      await page.keyboard.press("Shift+Enter");
      await page.keyboard.insertText("二行目");
      await page.waitForFunction(() => window.actions.some((item) => item.action === "draft" && item.value === "確認メッセージ\n二行目"));
      await expect(frame.getByRole("textbox", { name: "メッセージを入力", exact: true })).toContainText("二行目");
      ...
      await page.evaluate((composer) => window.sendPatch({ composer: { ...composer, text: "" } }), state.composer);
      await expect(frame.getByRole("textbox", { name: "メッセージを入力", exact: true })).toHaveText("");
      const incoming = frame.getByRole("list", { name: "メッセージ履歴" }).getByRole("button").filter({ hasText: "了解です。楽しみにしています！" });
      ...
      await page.mouse.click(bubble.x + bubble.width / 2, bubble.y + bubble.height / 2, { button: "right" });
      await page.waitForTimeout(150);
      assert.deepEqual(errors, [], "opening a message menu must not merge PaneTitle into the scrim");
      ...
      await page.waitForFunction(() => window.actions.some((item) => item.action === "draft" && item.value === "送信後の新しい下書き"));
      ...
      await page.waitForFunction(() => window.actions.some((item) => item.action === "appearance" && item.value === "dark"));
      await page.evaluate(() => window.sendPatch({ view: "chat" }));
    } else {
      await clickNative(frame.getByRole("button", { name: /グループを作成/ }));
      await page.waitForFunction(() => window.actions.some((item) => item.action === "create-group"));
      await clickNative(frame.getByRole("textbox", { name: "トークを検索", exact: true }));
      await page.keyboard.insertText("確認");
      await page.waitForFunction(() => window.actions.some((item) => item.action === "search" && item.value === "確認"));
    }
    ...
    if (chatMode && process.argv.includes("--profile")) {
      ...
      for (const width of [280, 768, 1280, 1920]) {
        ...
        if (mode === "apple") {
          const expectedCenter = width < 760 ? width / 2 : (Math.max(280, Math.min(384, width * .28)) + width) / 2;
          await expect.poll(async () => {
            const bounds = await frame.getByRole("button", { name: /高橋 美咲の情報/ }).boundingBox();
            return bounds ? Math.abs(bounds.x + bounds.width / 2 - expectedCenter) : Number.POSITIVE_INFINITY;
          }).toBeLessThan(10);
        } else await expect.poll(async () => (await frame.getByRole("button", { name: "設定", exact: true }).last().boundingBox())?.x ?? 0).toBeGreaterThan(width - 100);
        const bounds = await frame.locator("canvas").first().boundingBox();
        assert.ok(bounds && Math.abs(bounds.width - width) < 1, "canvas must resize to the viewport");
        await page.screenshot({ path: join(artifacts, `${mode}-${width}-dpr2.png`) });
      }
      await clickNative(frame.getByRole("textbox", { name: "メッセージを入力", exact: true }));
      const probe = frame.evaluate(() => new Promise((resolve) => {
        const intervals = []; const longTasks = [];
        ...
        function tick(time) {
          ...
          else { observer.disconnect(); resolve({ intervals, longTasks }); }
        }
      ...
      console.log(`${mode} DPR2 1920px scroll+typing: median=${timing.intervals[45].toFixed(1)}ms p95=${timing.intervals[85].toFixed(1)}ms longTasks=${timing.longTasks.length}`);
      await page.setViewportSize({ width: 390, height: 844 });
    }
    if (chatMode && process.argv.includes("--extended")) {
      const mediaMessages = [
        { ...state.messages.at(-2), id: "native-photo", kind: "image", text: "", mediaUrl: sampleSvg, fileName: "SVG photo" },
        { ...state.messages.at(-1), id: "native-sticker", kind: "sticker", text: "SVG sticker", mediaUrl: sampleSvg },
        { ...state.messages.at(-1), id: "native-animated-sticker", kind: "sticker", text: "animated sticker", mediaUrl: sampleSvg, stickerAnimated: true },
      ];
      const composer = { ...state.composer, pending: [{ id: "pending-image", name: "添付画像.svg", kind: "image", url: sampleSvg }] };
      await page.evaluate(({ messages, composer }) => window.sendPatch({ messages, composer, dark: false }), { messages: mediaMessages, composer });
      await page.waitForTimeout(800);
      assert.ok(await frame.locator("img").evaluateAll((images) => images.some((image) => image.naturalWidth > 0)), "Kotlin-owned animated sticker must decode SVG");
      assert.equal(await frame.locator('img[alt="SVG sticker"]').count(), 0, "static stickers must render in Skia for glass sampling");
      await page.screenshot({ path: join(artifacts, `${mode}-media.png`) });
      const readerMessage = { ...state.messages.at(-1), id: "readers-probe", text: "絵文字も確認 😀 👍 ❤️ 🎉", readCount: 2,
        readers: [{ id: "reader-1", name: "青木", readAt: 1788706800000 }, { id: "reader-2", name: "佐藤" }] };
      await page.evaluate(({ chat, message }) => window.sendPatch({ chat: { ...chat, isGroup: true }, messages: [message], composer: { text: "", available: true } }), { chat: state.chat, message: readerMessage });
      await clickNative(frame.getByRole("button", { name: "既読者一覧 2人", exact: true }));
      await page.waitForFunction(() => window.actions.some((item) => item.action === "readers" && item.id === "readers-probe"));
      await page.evaluate(() => window.sendPatch({ readersPanel: { messageId: "readers-probe", loading: false } }));
      ...
      await page.waitForFunction(() => window.actions.some((item) => item.action === "reader-profile" && item.id === "reader-1"));
      await page.keyboard.press("Escape");
      await page.waitForFunction(() => window.actions.some((item) => item.action === "close-readers"));
      await page.evaluate(({ messages, composer, chat }) => window.sendPatch({ messages, composer, chat, readersPanel: null }), { messages: mediaMessages, composer, chat: state.chat });
      await clickNative(frame.getByRole("button", { name: /添付画像.svgを削除/ }));
      await page.waitForFunction(() => window.actions.some((item) => item.action === "remove-attachment" && item.id === "pending-image"));
      await page.evaluate((composer) => window.sendPatch({ composer: { ...composer, text: "@", pending: [], mentionOptions: [{ all: true, name: "全員" }, { mid: "member-1", name: "高橋 美咲" }], mentionIndex: 0 } }), composer);
      await page.waitForTimeout(200);
      await clickNative(frame.getByRole("textbox", { name: "メッセージを入力", exact: true }));
      await page.keyboard.press("ArrowDown");
      await page.keyboard.press("Enter");
      await page.waitForFunction(() => window.actions.some((item) => item.action === "mention" && item.id === "1"));
      await page.evaluate(() => window.sendPatch({ chat: null, messages: [] }));
      await frame.getByRole("list", { name: "メッセージ履歴" }).waitFor({ state: "detached", timeout: 5000 });
      await page.screenshot({ path: join(artifacts, `${mode}-after-back.png`) });
      assert.equal(await frame.getByRole("list", { name: "メッセージ履歴" }).count(), 0, "a null chat patch must clear the native conversation");
      console.log(`${mode}: SVG/sticker, pending removal, native mention keyboard selection and null-chat patch passed`);
    }
    console.log(`${mode}: native ${chatMode ? "multiline draft/send/settings" : "create-group/search"}, Japanese semantics and 1200-row virtualization passed`);
  }
