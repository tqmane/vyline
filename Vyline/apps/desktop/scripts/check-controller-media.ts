...
try {
  await page.route("**/*", (route) => {
    ...
    if (url.pathname.startsWith("/api/"))
      return route.fulfill({ json: { ok: true, chats: [], members: [], results: [] } });
    return route.continue();
  });
  await page.addInitScript(() =>
    localStorage.setItem(
      "vyline:design-system",
      JSON.stringify({ state: { mode: "apple", appearance: "dark" }, version: 0 }),
    ),
  );
  await page.goto(`${base}/pr-demo`);
  await expect(page.locator('[data-kmp-ready="true"]')).toBeVisible({ timeout: 60000 });
  await page.evaluate(async () => {
    // Reuse Vite's already-loaded module URL, including its HMR version, so
    // the fixture and CallVideoStage share the same React context instance.
    const load = (path: string) => import(performance.getEntriesByType("resource").findLast((entry) => new URL(entry.name).pathname === path)?.name ?? path);
    ...
    context.fillStyle = "#167cd3";
    context.fillRect(0, 0, 320, 180);
    const stream = source.captureStream(10);
    const host = document.createElement("div");
    document.body.append(host);
    const root = createRoot(host);
    const probe = ((window as any).__mediaProbe = {
      original: null,
      attaches: 0,
      detaches: 0,
      stream,
    ...
    const render = () =>
      root.render(
        React.createElement(
          ...
          { native: probe.native, title: "映像の保持検証", onClose: () => {} },
          React.createElement(CallVideoStage, {
            tiles: [
              { id: "one", name: "カメラ", visible: true, content: video },
              {
                id: "two",
                name: "画面共有",
                visible: true,
                content: React.createElement(
                  "div",
                  { style: { background: "#a82660", height: "100%" } },
                  "画面共有",
                ),
              },
            ],
    ...
    probe.render = (native: boolean) => {
      probe.native = native;
      host.style.cssText = native
        ? ""
        : "position:fixed;inset:0;z-index:200;background:black;display:flex";
      render();
    };
  ...
  const split = frame.getByRole("button", { name: "分割", exact: true });
  ...
  for (const mode of ["fluent", "miuix", "apple"]) {
    ...
    await expect
      .poll(() =>
        page.evaluate(() => {
          const p = (window as any).__mediaProbe;
          return (
            p.original.isConnected &&
            p.original.srcObject === p.stream &&
            p.attaches === 1 &&
            p.detaches === 0
          );
        }),
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      attaches: p.attaches,
      detaches: p.detaches,
      retainedStream: p.original.srcObject === p.stream,
      visible: p.original.isConnected,
    };
  });
  ...
  console.log(
    `Controller media PASS: native layout controls, three themes, native/legacy reparenting retain the same video and stream. ${output}`,
  );
} catch (error) {
