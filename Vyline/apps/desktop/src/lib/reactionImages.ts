...
export function reactionSticonUrl(type: number, emoji?: { productId: string; emojiId: string }): string {
  ...
  // The account-free demo has no BFF; only public reaction assets load directly.
  return typeof window !== "undefined" && window.location.pathname === "/pr-demo" ? url : lineCdnProxy(url);
}
